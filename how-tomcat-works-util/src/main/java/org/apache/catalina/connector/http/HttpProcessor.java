...
final class HttpProcessor
     ...
     */
    synchronized void assign(Socket socket) {//注意这里assign和await的配合，await是为了空闲的processor
                                             //不空转而阻塞，当assign接收到一个请求时，将available设置为
                                             //true，使processor开始处理请求
    ...
    private synchronized Socket await() {
        ...
        Socket socket = this.socket;//使用局部变量可以在当前Socket处理完之前继续接收下一个Socket
        available = false;
        notifyAll();
    ...
    private void process(Socket socket) {
        ...
        while (!stopped && ok && keepAlive) {
            ...
            try {
                ...
                if (ok) {
                    //解析一切ok的时候，将request和response作为参数传入容器中
